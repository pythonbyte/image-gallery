...
class PhotoForm(forms.ModelForm):
    friends_name = forms.CharField(label="Image Name", widget=forms.TextInput(
        attrs={"class": 'input-field col s12'}))
    date_taken = forms.DateField(widget=forms.TextInput(
    ...
    def clean_image_file(self):
        ...
        if (not isinstance(image_file, InMemoryUploadedFile) or
                image_file.content_type != 'image/jpeg'):
            raise forms.ValidationError('Incorrect type of image, only .JPG/.JPEG allowed.')
        return image_file
