...
class TestFormImage(TestCase):
    ...
    def test_success_data(self):
        success_data = {"friends_name": "john", "date_taken": "2019-09-25"}
        form = self.form_class(success_data, {"image_file": self.image})

        self.assertTrue(form.is_valid())

    def test_missing_file(self):
        data = {"friends_name": "john", "date_taken": "2019-09-25"}
    ...
    def test_invalid_file(self):
        ...
        self.assertEqual(
            form.errors,
            {"image_file": ["Incorrect type of image, only .JPG/.JPEG allowed."]},
        )

    def test_missing_friends_name(self):
        data = {"date_taken": "2019-09-25"}
    ...
    def test_numeric_friends_name(self):
        data = {"friends_name": "123456", "date_taken": "2019-09-25"}
        form = self.form_class(data, {"image_file": self.image})

        self.assertFalse(form.is_valid())
        self.assertEqual(
            form.errors, {"friends_name": ["Invalid name. Please provide a valid one."]}
        )

    def test_missing_date_taken(self):
        data = {"friends_name": "john"}
        form = self.form_class(data, {"image_file": self.image})
    ...
    def test_invalid_date_taken(self):
        data = {"friends_name": "john", "date_taken": "2019-29-25"}
        form = self.form_class(data, {"image_file": self.image})
...
class TestPhotoEndpoints(TestCase):
    ...
    def test_like_photo_not_ajax(self):
        ...
        response = self.client.post(
            reverse("imageapp:like-picture"), data={"image_id": photo.id}
        )
